...
#### このフェーズで出来るようになること
- メッセージの永続化: ページを再読込みしてもメッセージが残る
- 過去の会話履歴表示: データベースに保存された過去のメッセージを表示
- 基本的なDM機能: 二人のユーザー間でのメッセージ送受信が可能
...
- オンライン状態表示:ユーザーがオンライン/オフラインかを表示
- 未読メッセージ数表示:各ユーザーからの未読メッセージ数をカウント

#### このフェーズの価値
- LINE/WhatsAppのような本格的なチャットアプリに近づく
